...
int main(void)
{
    ...
    //game_state->camera.projection = CAMERA_ORTHOGRAPHIC;        // Camera projection type
    ...
    while (!WindowShouldClose())                               // Detect window close button or ESC key
    {
        ...
        DrawLine3D(game_state->room[ROOM_1].wall[0].start,
                   game_state->room[ROOM_1].wall[0].end,
                   RED);
        
        DrawLine3D(game_state->room[ROOM_1].wall[1].start,
                   game_state->room[ROOM_1].wall[1].end,
                   RED);
        ...
        DrawText(TextFormat("Yaw %.2f",game_state->player.yaw),     10,  100, 20, BLACK);
        
        DrawText(TextFormat("Animation: %s\n",
                            game_state->player.anim[game_state->player.anim_index].name),
                 10,  120, 20, BLACK);
